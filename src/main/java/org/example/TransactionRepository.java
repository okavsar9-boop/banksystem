...
public class TransactionRepository {
    public static void insertTransaction(Connection connection, String senderPassportId, String receiverPassportId, String transaction_type, Double amount) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement("insert into transactions (sender_passport_id,receiver_passport_id,action_type,amount) values (?,?,?::transaction_type,?)")) {
    ...
    public static Double getBalance(Connection connection, String passportId) throws SQLException{
        try(PreparedStatement ps = connection.prepareStatement("select Coalesce(sum(case when action_type = 'DEPOSIT' then amount else -amount end), 0) from transactions where sender_passport_id = ?")){
